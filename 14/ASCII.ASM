...
B20:
		LEA		SI,DISPROW	; Выводимый текст
...
C10HEX	PROC	NEAR
		...
		SHR		AL,CL
		AND		AH,0FH
		LEA		BX,XLATAB	; Перекодировать и сохранить.
		XLAT
		MOV		[SI],AL
		MOV		AL,AH
		XLAT
...
D10BORD	PROC
		...
		LEA		SI,SPACE	; Область заголовка
		MOV		DH,1
		CALL	P10DISP

		LEA		SI,BORDER	; Граница
...
D20:	ADD		DL,5
		...
		LEA		SI,MNODE	; Средний правый узел
		CALL	P10DISP

		LEA		SI,HEXSTR+1	; Правая граница заголовка
		DEC		DH
		CALL	P10DISP

		LEA		SI,HCORNER	; Верхний правый угол
		DEC		DH
		CALL	P10DISP

		LEA		SI,TNAME	; Заголовок
		MOV		CX,1
		MOV		DX,0116H
D30:	INC		DL
		CALL	P10DISP
...
D40:	ADD		DL,5
		...
		LEA		SI,LCORNER	; Нижний правый угол
		CALL	P10DISP
